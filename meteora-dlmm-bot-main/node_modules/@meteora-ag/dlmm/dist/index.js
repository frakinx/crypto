...
async function chunkDepositWithRebalanceEndpoint(dlmm, strategy, slippagePercentage, maxActiveBinSlippage, position, positionMinBinId, positionMaxBinId, liquidityStrategyParameters, owner, payer, isParallel, skipSolWrappingOperation = false) {
  ...
  for (let i = 0; i < chunkedBinRange.length; i++) {
    ...
    if (dlmm.tokenX.publicKey.equals(_spltoken.NATIVE_MINT) && !skipSolWrappingOperation) {
    ...
    if (dlmm.tokenY.publicKey.equals(_spltoken.NATIVE_MINT) && !skipSolWrappingOperation) {
      const wrapSOLIx = wrapSOLInstruction(
        owner,
        userTokenY,
    ...
    if (dlmm.tokenX.publicKey.equals(_spltoken.NATIVE_MINT) && !totalXAmount.isZero() && !skipSolWrappingOperation) {
    ...
    if (dlmm.tokenY.publicKey.equals(_spltoken.NATIVE_MINT) && !totalYAmount.isZero() && !skipSolWrappingOperation) {
      addLiquidityIxs.push(
        _spltoken.createCloseAccountInstruction.call(void 0, 
          userTokenY,
          owner,
...
var DLMM = class _DLMM {
  ...
  async initializeMultiplePositionAndAddLiquidityByStrategy2(positionKeypairGenerator, totalXAmount, totalYAmount, strategy, owner, payer, slippagePercentage, altAddress) {
    ...
    const defaultAltAddressString = _nullishCoalesce(altAddress, () => ( ALT_ADDRESS[_optionalChain([this, 'access', _85 => _85.opt, 'optionalAccess', _86 => _86.cluster])]));
  ...
  async addLiquidityByWeight({
    ...
    if ([
      this.tokenX.publicKey.toBase58(),
      this.tokenY.publicKey.toBase58()
    ].includes(_spltoken.NATIVE_MINT.toBase58()) && !_optionalChain([this, 'access', _115 => _115.opt, 'optionalAccess', _116 => _116.skipSolWrappingOperation])) {
      const closeWrappedSOLIx = await unwrapSOLInstruction(user);
      closeWrappedSOLIx && postInstructions.push(closeWrappedSOLIx);
    }
    const liquidityParams = {
  ...
  async removeLiquidity({
    ...
    for (const { lowerBinId, upperBinId } of chunkedBinRange) {
      ...
      if ([
        this.tokenX.publicKey.toBase58(),
        this.tokenY.publicKey.toBase58()
      ].includes(_spltoken.NATIVE_MINT.toBase58()) && (!skipUnwrapSOL || !_optionalChain([this, 'access', _117 => _117.opt, 'optionalAccess', _118 => _118.skipSolWrappingOperation]))) {
        const closeWrappedSOLIx = await unwrapSOLInstruction(user);
        closeWrappedSOLIx && postInstructions.push(closeWrappedSOLIx);
      }
  ...
  async swap({
    ...
    createInTokenAccountIx && preInstructions.push(createInTokenAccountIx);
    createOutTokenAccountIx && preInstructions.push(createOutTokenAccountIx);
    if (inToken.equals(_spltoken.NATIVE_MINT) && !_optionalChain([this, 'access', _135 => _135.opt, 'optionalAccess', _136 => _136.skipSolWrappingOperation])) {
      const wrapSOLIx = wrapSOLInstruction(
        user,
        userTokenIn,
        BigInt(inAmount.toString())
    ...
    if (outToken.equals(_spltoken.NATIVE_MINT) && !_optionalChain([this, 'access', _137 => _137.opt, 'optionalAccess', _138 => _138.skipSolWrappingOperation])) {
      const closeWrappedSOLIx = await unwrapSOLInstruction(user);
      closeWrappedSOLIx && postInstructions.push(closeWrappedSOLIx);
    }
    const binArrays = binArraysPubkey.map((pubkey) => {
  ...
  async syncWithMarketPrice(marketPrice, owner) {
    ...
    if (!!_optionalChain([binArrayAccounts, 'optionalAccess', _144 => _144[1]])) {
      fromBinArray = fromBinArrayPubkey;
    }
    if (!!_optionalChain([binArrayAccounts, 'optionalAccess', _145 => _145[2]]) && !!toBinArrayIndex) {
  ...
  async rebalancePosition(rebalancePositionResponse, maxActiveBinSlippage, rentPayer, slippage = 100) {
    ...
    const postInstructions = [];
    if (this.tokenX.publicKey.equals(_spltoken.NATIVE_MINT) && simulationResult.actualAmountXDeposited.gtn(0) && !_optionalChain([this, 'access', _146 => _146.opt, 'optionalAccess', _147 => _147.skipSolWrappingOperation])) {
      const wrapSOLIx = wrapSOLInstruction(
        owner,
        userTokenX,
    ...
    if (this.tokenY.publicKey.equals(_spltoken.NATIVE_MINT) && simulationResult.actualAmountYDeposited.gtn(0) && !_optionalChain([this, 'access', _148 => _148.opt, 'optionalAccess', _149 => _149.skipSolWrappingOperation])) {
      const wrapSOLIx = wrapSOLInstruction(
        owner,
        userTokenY,
    ...
    if ((this.tokenX.publicKey.equals(_spltoken.NATIVE_MINT) || this.tokenY.publicKey.equals(_spltoken.NATIVE_MINT)) && !_optionalChain([this, 'access', _150 => _150.opt, 'optionalAccess', _151 => _151.skipSolWrappingOperation])) {
  ...
  async getBins(lbPairPubKey, lowerBinId, upperBinId, baseTokenDecimal, quoteTokenDecimal, lowerBinArray, upperBinArray) {
    ...
    const version = _nullishCoalesce(_optionalChain([binArrays, 'access', _152 => _152.find, 'call', _153 => _153((binArray) => binArray != null), 'optionalAccess', _154 => _154.version]), () => ( 1));
    return Array.from(
      enumerateBins(
        binsById,
        lowerBinId,
