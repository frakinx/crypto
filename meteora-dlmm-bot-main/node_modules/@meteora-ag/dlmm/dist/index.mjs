...
var DEFAULT_BIN_PER_POSITION = new BN(
...
var POSITION_MAX_LENGTH = new BN(
...
import { AnchorProvider, BN as BN18, Program as Program2 } from "@coral-xyz/anchor";
import {
...
async function getExtraAccountMetasForTransferHook(connection, mintAddress, mintAccountInfo) {
  ...
  if (!transferHook || transferHook.programId.equals(PublicKey3.default)) {
    return [];
  } else {
...
function calculatePreFeeAmount(transferFee, postFeeAmount) {
  ...
  const maximumFee = new BN4(transferFee.maximumFee.toString());
  if (transferFee.transferFeeBasisPoints === MAX_FEE_BASIS_POINTS) {
    return postFeeAmount.add(maximumFee);
  }
  const ONE_IN_BASIS_POINTS = new BN4(MAX_FEE_BASIS_POINTS);
  const numerator = postFeeAmount.mul(ONE_IN_BASIS_POINTS);
  const denominator = ONE_IN_BASIS_POINTS.sub(
    new BN4(transferFee.transferFeeBasisPoints)
  );
  const rawPreFeeAmount = numerator.add(denominator).sub(new BN4(1)).div(denominator);
...
function calculateTransferFeeIncludedAmount(transferFeeExcludedAmount, mint, currentEpoch) {
  ...
  const transferFee = epochFee.transferFeeBasisPoints == MAX_FEE_BASIS_POINTS ? new BN4(epochFee.maximumFee.toString()) : calculateInverseFee(epochFee, transferFeeExcludedAmount);
...
function calculateTransferFeeExcludedAmount(transferFeeIncludedAmount, mint, currentEpoch) {
  ...
  const transferFeeExcludedAmount = new BN4(
  ...
  return distributions.map((bin) => {
    if (bin.binId > activeId) {
      ...
      return {
        binId: bin.binId,
        amount: new BN5(
          new Decimal3(totalAmount.toString()).mul(new Decimal3(bin.weight).div(totalWeight)).floor().toString()
        )
      };
    }
  });
  ...
  return distributions.map((bin) => {
    if (bin.binId < activeId) {
      ...
      return {
        binId: bin.binId,
        amount: new BN5(
          new Decimal3(totalAmount.toString()).mul(weightPerPrice).div(totalWeight).floor().toString()
        )
      };
    }
  });
...
function toAmountBothSide(activeId, binStep, amountX, amountY, amountXInActiveBin, amountYInActiveBin, distributions, mintX, mintY, clock) {
  ...
  if (activeBins.length === 1) {
    ...
    if (amountXInActiveBin.isZero() && amountYInActiveBin.isZero()) {
      ...
      let amountXInActiveBinDec = new Decimal3(amountXInActiveBin.toString());
      let amountYInActiveBinDec = new Decimal3(amountYInActiveBin.toString());
      if (!amountXInActiveBin.isZero()) {
        wx0 = new Decimal3(activeBin.weight).div(
          p0.add(amountYInActiveBinDec.div(amountXInActiveBinDec))
        );
      }
      if (!amountYInActiveBin.isZero()) {
        wy0 = new Decimal3(activeBin.weight).div(
          new Decimal3(1).add(
    ...
    distributions.forEach((element) => {
      ...
      if (element.binId > activeId) {
        ...
        let weighPerPrice = new Decimal3(element.weight).div(price);
    ...
    return distributions.map((bin) => {
      if (bin.binId < activeId) {
        const amount = k.mul(new Decimal3(bin.weight));
        return {
          binId: bin.binId,
          amountX: new BN5(0),
          amountY: new BN5(amount.floor().toString())
        };
      }
      if (bin.binId > activeId) {
        const price = getPriceOfBinByBinId(bin.binId, binStep);
        ...
        return {
          binId: bin.binId,
          amountX: new BN5(amount.floor().toString()),
          amountY: new BN5(0)
        };
      }
      ...
      return {
        binId: bin.binId,
        amountX: new BN5(amountXActiveBin.floor().toString()),
        amountY: new BN5(amountYActiveBin.floor().toString())
      };
    });
  } else {
    ...
    let kx = new Decimal3(amountX.toString()).div(totalWeightX);
    let ky = new Decimal3(amountY.toString()).div(totalWeightY);
    let k = kx.lessThan(ky) ? kx : ky;
    return distributions.map((bin) => {
      if (bin.binId < activeId) {
        const amount = k.mul(new Decimal3(bin.weight));
        ...
        let weighPerPrice = new Decimal3(bin.weight).div(price);
        const amount = k.mul(weighPerPrice);
        return {
          binId: bin.binId,
          amountX: new BN5(amount.floor().toString()),
...
function getPriceOfBinByBinId(binId, binStep) {
  ...
  return new Decimal4(1).add(new Decimal4(binStepNum)).pow(new Decimal4(binId));
}
...
function computeAllocationBps(allocations) {
  let totalAllocation = new BN6(0);
  const bpsAllocations = [];
  for (const allocation of allocations) {
    const allocBps = new BN6(allocation * 1e4);
    bpsAllocations.push(allocBps);
    totalAllocation = totalAllocation.add(allocBps);
  }
  const pLoss = new BN6(1e4).sub(totalAllocation);
  return {
    bpsAllocations,
    pLoss
  };
...
function toWeightDistribution(amountX, amountY, distributions, binStep) {
  let totalQuote = new BN6(0);
  const precision = 1e12;
  const quoteDistributions = distributions.map((bin) => {
    const price = new BN6(
      getPriceOfBinByBinId(bin.binId, binStep).mul(precision).floor().toString()
    );
    const quoteValue = amountX.mul(new BN6(bin.xAmountBpsOfTotal)).mul(new BN6(price)).div(new BN6(BASIS_POINT_MAX)).div(new BN6(precision));
    const quoteAmount = quoteValue.add(
      amountY.mul(new BN6(bin.yAmountBpsOfTotal)).div(new BN6(BASIS_POINT_MAX))
    );
  ...
  if (totalQuote.eq(new BN6(0))) {
    return [];
  }
  const distributionWeights = quoteDistributions.map((bin) => {
    const weight = Math.floor(
      bin.quoteAmount.mul(new BN6(65535)).div(totalQuote).toNumber()
    );
    return {
      binId: bin.binId,
      weight
...
function calculateSpotDistribution(activeBin, binIds) {
  if (!binIds.includes(activeBin)) {
    const { div: dist, mod: rem } = new BN6(1e4).divmod(
      new BN6(binIds.length)
    );
    const loss = rem.isZero() ? new BN6(0) : new BN6(1);
    const distributions = binIds[0] < activeBin ? binIds.map((binId) => ({
      binId,
      xAmountBpsOfTotal: new BN6(0),
      yAmountBpsOfTotal: dist
    })) : binIds.map((binId) => ({
      binId,
      xAmountBpsOfTotal: dist,
      yAmountBpsOfTotal: new BN6(0)
    }));
  ...
  const yBinBps = new BN6(1e4 / totalYBinCapacity);
  const yActiveBinBps = new BN6(1e4).sub(yBinBps.mul(new BN6(binYCount)));
  const xBinBps = new BN6(1e4 / totalXBinCapacity);
  const xActiveBinBps = new BN6(1e4).sub(xBinBps.mul(new BN6(binXCount)));
...
function calculateBidAskDistribution(activeBin, binIds) {
  ...
  const [normXAllocations, normYAllocations] = allocations.reduce(
    ([xAllocations, yAllocations], allocation, idx) => {
      const binId = binIds[idx];
      if (binId > activeBin) {
        const distX = new BN6(allocation * 1e4 / totalXAllocation);
        xAllocations.push(distX);
      }
      if (binId < activeBin) {
        const distY = new BN6(allocation * 1e4 / totalYAllocation);
        yAllocations.push(distY);
      }
      if (binId == activeBin) {
        const half = allocation / 2;
        const distX = new BN6(half * 1e4 / totalXAllocation);
        const distY = new BN6(half * 1e4 / totalYAllocation);
        xAllocations.push(distX);
        yAllocations.push(distY);
      }
  ...
  const xPLoss = new BN6(1e4).sub(totalXNormAllocations);
  const yPLoss = new BN6(1e4).sub(totalYNormAllocations);
  ...
  if (activeId > distributions[distributions.length - 1].binId) {
    ...
    return amounts.map((bin) => {
      return {
        binId: bin.binId,
        amountX: new BN6(0),
        amountY: new BN6(bin.amount.toString())
      };
    });
  }
  if (activeId < distributions[0].binId) {
    ...
    return amounts.map((bin) => {
      return {
        binId: bin.binId,
        amountX: new BN6(bin.amount.toString()),
        amountY: new BN6(0)
      };
    });
  }
...
function computeBaseFactorFromFeeBps(binStep, feeBps) {
  ...
  if (computedBaseFactor > U16_MAX) {
    ...
    return [new BN7(truncatedBaseFactor), new BN7(base_power_factor)];
  } else {
    ...
    return [new BN7(computedBaseFactor), new BN7(0)];
  }
}
...
function getPositionCount(minBinId, maxBinId) {
  ...
  return positionCount.add(new BN7(1));
}
function findOptimumDecompressMultiplier(binAmount, tokenDecimal) {
  let multiplier = new BN7(10).pow(tokenDecimal);
  while (!multiplier.isZero()) {
    ...
    for (const [_binId, amount] of binAmount) {
      ...
      if (compressedAmount.isZero()) {
        multiplier = multiplier.div(new BN7(10));
        found = false;
        break;
      }
    }
...
function compressBinAmount(binAmount, multiplier) {
  ...
  let totalAmount = new BN7(0);
  let compressionLoss = new BN7(0);
...
function generateAmountForBinRange(amount, binStep, tokenXDecimal, tokenYDecimal, minBinId, maxBinId, k) {
  const toTokenMultiplier = new Decimal5(10 ** (tokenXDecimal - tokenYDecimal));
...
function isRewardInitialized(reward) {
  return !reward.mint.equals(PublicKey5.default);
}
...
function updateBinArray(activeId, clock, allRewardInfos, binArray) {
  ...
  for (const [rewardIdx, reward] of allRewardInfos.entries()) {
    ...
    const rewardPerTokenStoredDelta = reward.rewardRate.mul(delta).div(new BN8(15)).div(liquiditySupply);
...
function getBaseFee(binStep, sParameter) {
  return new BN11(sParameter.baseFactor).mul(new BN11(binStep)).mul(new BN11(10)).mul(new BN11(10).pow(new BN11(sParameter.baseFeePowerFactor)));
}
function getVariableFee(binStep, sParameter, vParameter) {
  if (sParameter.variableFeeControl > 0) {
    const square_vfa_bin = new BN11(vParameter.volatilityAccumulator).mul(new BN11(binStep)).pow(new BN11(2));
    const v_fee = new BN11(sParameter.variableFeeControl).mul(square_vfa_bin);
    return v_fee.add(new BN11(99999999999)).div(new BN11(1e11));
  }
  return new BN11(0);
}
...
function computeFee(binStep, sParameter, vParameter, inAmount) {
  ...
  return inAmount.mul(totalFee).add(denominator).sub(new BN11(1)).div(denominator);
}
function computeFeeFromAmount(binStep, sParameter, vParameter, inAmountWithFees) {
  const totalFee = getTotalFee(binStep, sParameter, vParameter);
  return inAmountWithFees.mul(totalFee).add(FEE_PRECISION.sub(new BN11(1))).div(FEE_PRECISION);
}
function computeProtocolFee(feeAmount, sParameter) {
  return feeAmount.mul(new BN11(sParameter.protocolShare)).div(new BN11(BASIS_POINT_MAX));
}
function swapExactOutQuoteAtBin(bin, binStep, sParameter, vParameter, outAmount, swapForY) {
  if (swapForY && bin.amountY.isZero()) {
    return {
      amountIn: new BN11(0),
      amountOut: new BN11(0),
      fee: new BN11(0),
      protocolFee: new BN11(0)
    };
  }
  ...
  return positionData.positionBinData.map(
    ({
      ...
      return {
        ...
        price,
        pricePerToken,
        amountX: new BN12(positionXAmount),
        amountY: new BN12(positionYAmount),
        claimableRewardAmount: positionRewardAmount.map(
          (amount) => new BN12(amount)
        ),
        claimableFeeXAmount: new BN12(positionFeeXAmount),
        claimableFeeYAmount: new BN12(positionFeeYAmount)
      };
    }
  );
}
...
function onlyDepositToBidSide(maxDeltaId, favorXInActiveBin) {
  if (favorXInActiveBin) {
    return maxDeltaId.lt(new BN12(0));
  }
  return maxDeltaId.lte(new BN12(0));
}
function onlyDepositToAskSide(minDeltaId, favorXInActiveBin) {
  if (favorXInActiveBin) {
    return minDeltaId.gte(new BN12(0));
  }
  return minDeltaId.gt(new BN12(0));
}
function getAmountInBinsBidSide(activeId, minDeltaId, maxDeltaId, deltaY, y0) {
  ...
  for (let binId = minBinId.toNumber(); binId <= maxBinId.toNumber(); binId++) {
    ...
    const totalDeltaY = deltaY.mul(new BN12(deltaBin));
    const amountY = y0.add(totalDeltaY);
    amountInBins.push({
      binId: new BN12(binId),
      amountX: new BN12(0),
      amountY
    });
  }
...
function getAmountInBinsAskSide(activeId, binStep, minDeltaId, maxDeltaId, deltaX, x0) {
  const binCount = maxDeltaId.sub(minDeltaId).add(new BN12(1));
  const minBinId = activeId.add(minDeltaId);
  const maxBinId = activeId.add(maxDeltaId);
  ...
  for (let binId = maxBinId.toNumber(); binId >= minBinId.toNumber(); binId--) {
    ...
    const totalDeltaX = deltaX.mul(new BN12(delta));
...
function computeCompositionFee(binStep, sParameters3, vParameters3, outAmountX, inAmountX, outAmountY, inAmountY) {
  if (outAmountX.gt(inAmountX)) {
    ...
    return feeAmount.mul(FEE_PRECISION.add(totalFeeRate)).div(FEE_PRECISION.pow(new BN12(2)));
  }
  return new BN12(0);
}
...
var RebalancePosition = class _RebalancePosition {
  ...
  constructor(positionAddress, positionData, lbPair, activeBin, shouldClaimFee, shouldClaimReward, currentTimestamp) {
    ...
    this.lowerBinId = new BN12(positionData.lowerBinId);
    this.upperBinId = new BN12(positionData.upperBinId);
    this.lbPair = lbPair;
  ...
  static async create(params) {
    ...
    const activeBinArrayIdx = binIdToBinArrayIndex(new BN12(lbPair.activeId));
  ...
  _simulateDeposit(binStep, tokenXDecimal, tokenYDecimal, deposits, simulatedWithdrawResult) {
    ...
    const activeId = new BN12(this.lbPair.activeId);
    ...
    if (depositBinIds.length > 0) {
      ...
      this._simulateResize(
        new BN12(depositMinBinId),
        new BN12(depositMaxBinId),
        binStep,
        tokenXDecimal,
        tokenYDecimal
      );
    }
    let totalAmountXDeposited = new BN12(0);
    let totalAmountYDeposited = new BN12(0);
    ...
    if (newMinBinId.lt(this.lowerBinId)) {
      ...
      for (let i = 1; i <= binCountToExpand.toNumber(); i++) {
        ...
        this.rebalancePositionBinData.unshift({
          ...
          amountX: new BN12(0),
          amountY: new BN12(0),
          claimableRewardAmount: [new BN12(0), new BN12(0)],
          claimableFeeXAmount: new BN12(0),
          claimableFeeYAmount: new BN12(0)
        });
      }
    } else {
  ...
  _simulateWithdraw(withdraws) {
    let liquidityAndFeeXWithdrawn = new BN12(0);
    let liquidityAndFeeYWithdrawn = new BN12(0);
    let rewardsAmountClaimed = [new BN12(0), new BN12(0)];
    const activeId = new BN12(this.lbPair.activeId);
    for (const { minBinId, maxBinId, bps } of withdraws) {
      ...
      for (const binId of binIds) {
        ...
        if (this.shouldClaimFee) {
          ...
          binData.claimableFeeXAmount = new BN12(0);
          binData.claimableFeeYAmount = new BN12(0);
        }
        if (this.shouldClaimReward) {
          for (const [idx2, amount] of binData.claimableRewardAmount.entries()) {
            ...
            binData.claimableRewardAmount[idx2] = new BN12(0);
          }
        }
  ...
  async simulateRebalance(connection, binStep, tokenXDecimal, tokenYDecimal, withdraws, deposits) {
    ...
    let rentalCostLamports = new BN12(0);
    if (widthDelta != 0) {
      ...
      const lamportChanges = new BN12(rentExemptionLamports).sub(
        new BN12(minimumLamports)
      );
      if (widthDelta > 0) {
  ...
  totalAmounts() {
    let totalAmountX = new BN12(0);
    let totalAmountY = new BN12(0);
  ...
  totalFeeAmounts() {
    let totalFeeXAmount = new BN12(0);
    let totalFeeYAmount = new BN12(0);
    for (const binData of this.rebalancePositionBinData) {
  ...
  totalRewardAmounts() {
    let totalRewardAmounts = [new BN12(0), new BN12(0)];
    for (const binData of this.rebalancePositionBinData) {
...
function binRangeToBinIdArray(minBinId, maxBinId) {
  ...
  for (let binId = fromBinId; binId <= toBinId; binId++) {
    binIdArray.push(new BN12(binId));
  }
  return binIdArray;
}
function getRebalanceBinArrayIndexesAndBitmapCoverage(adds, removes, activeId, pairAddress, programId) {
  ...
  const requireBitmapExtension = binArrayIndexes.some(
    (index) => isOverflowDefaultBinArrayBitmap(new BN12(index))
  );
  return {
    binArrayIndexes,
...
function findY0AndDeltaY(amountY, minDeltaId, maxDeltaId, activeId) {
  ...
  while (true) {
    ...
    if (totalAmountY.gt(amountY)) {
      baseDeltaY = baseDeltaY.sub(new BN13(1));
    } else {
      return {
        base: y0,
        delta: baseDeltaY
...
function findBaseY0(amountY, minDeltaId, maxDeltaId) {
  if (minDeltaId.gt(maxDeltaId) || amountY.lte(new BN14(0))) {
    return new BN14(0);
  }
  if (minDeltaId.eq(maxDeltaId)) {
    return amountY;
...
function findBaseX0(amountX, minDeltaId, maxDeltaId, binStep, activeId) {
  if (minDeltaId.gt(maxDeltaId) || amountX.lte(new BN14(0))) {
    return new BN14(0);
  }
  let b = new BN14(0);
  let c = new BN14(0);
  ...
  for (let m = m1.toNumber(); m <= m2.toNumber(); m++) {
    ...
    const pm = getQPriceFromId(binId.neg(), binStep);
    b = b.add(pm);
    const cDelta = new BN14(m).mul(pm).div(m2);
    c = c.add(cDelta);
  }
...
function findY0(amountY, minDeltaId, maxDeltaId) {
  if (minDeltaId.gt(maxDeltaId) || amountY.lte(new BN15(0)) || amountY.isZero()) {
    return new BN15(0);
  }
  const m1 = minDeltaId.neg();
  const m2 = maxDeltaId.neg();
...
function findX0(amountX, minDeltaId, maxDeltaId, binStep, activeId) {
  ...
  while (true) {
    ...
    if (totalAmountX.lt(amountX)) {
      x0 = x0.add(new BN15(1));
    } else {
      x0 = x0.sub(new BN15(1));
      return x0;
    }
  }
}
...
function suggestBalancedXParametersFromY(y0, deltaY, minDeltaId, maxDeltaId, activeId, binStep, favorXInActiveBin, builder) {
  const endDeltaIdBidSide = favorXInActiveBin ? new BN16(-1) : new BN16(0);
...
function getAutoFillAmountByRebalancedPosition(rebalancePosition, strategyType) {
  let liquidityInBidSide = new BN16(0);
  let liquidityInAskSide = new BN16(0);
  ...
  if (liquidityInAskSide.gt(liquidityInBidSide)) {
    ...
    const { amountY } = builder.suggestBalancedYParametersFromX(
      new BN16(lbPair.activeId),
      new BN16(lbPair.binStep),
      favorXInActiveBin,
      new BN16(minDeltaId),
      new BN16(maxDeltaId),
      liquidityInAskSide
    ...
    return {
      amount: BN16.max(amountY.sub(positionAmountY), new BN16(0)),
      isBidSide: true
    };
    ...
    return {
      amount: BN16.max(amountX.sub(positionAmountX), new BN16(0)),
      isBidSide: false
    };
  } else {
    return {
...
function suggestBalancedYParametersFromX(x0, deltaX, minDeltaId, maxDeltaId, activeId, binStep, favorXInActiveBin, builder) {
  const startDeltaIdAskSide = favorXInActiveBin ? new BN16(0) : new BN16(1);
...
function buildLiquidityStrategyParameters(amountX, amountY, minDeltaId, maxDeltaId, binStep, favorXInActiveId, activeId, strategyParameterBuilder) {
  ...
  const depositOnlyY = maxDeltaId.lt(new BN16(0)) || maxDeltaId.isZero() && !favorXInActiveId;
  const depositOnlyX = minDeltaId.gt(new BN16(0)) || minDeltaId.isZero() && favorXInActiveId;
  if (depositOnlyY) {
  ...
  const maxDeltaIdBidSide = favorXInActiveId ? new BN16(-1) : new BN16(0);
  const minDeltaIdAskSide = favorXInActiveId ? new BN16(0) : new BN16(1);
...
import { BN as BN17 } from "@coral-xyz/anchor";
...
function resetUninvolvedLiquidityParams(minDeltaId, maxDeltaId, favorXInActiveId, params) {
  const endBidSideDeltaId = favorXInActiveId ? new BN18(-1) : new BN18(0);
...
async function chunkDepositWithRebalanceEndpoint(dlmm, strategy, slippagePercentage, maxActiveBinSlippage, position, positionMinBinId, positionMaxBinId, liquidityStrategyParameters, owner, payer, isParallel, skipSolWrappingOperation = false) {
  ...
  for (let i = 0; i < chunkedBinRange.length; i++) {
    ...
    const binArrayIndexes = getBinArrayIndexesCoverage(
      new BN18(chunkMinBinId),
      new BN18(chunkMaxBinId)
    );
    ...
    const { totalXAmount, totalYAmount } = toAmountIntoBins(
      ...
      {
        totalXAmount: new BN18(0),
        totalYAmount: new BN18(0)
      }
    );
    ...
    const rebalanceIx = await dlmm.program.methods.rebalanceLiquidity(
      {
        ...
        minWithdrawXAmount: new BN18(0),
        minWithdrawYAmount: new BN18(0),
        maxDepositXAmount,
        maxDepositYAmount,
        removes: [],
        adds: [addParam],
...
var MAX_BPS = new BN19(BASIS_POINT_MAX);
function capBps(bps) {
  return bps.lt(new BN19(0)) ? new BN19(0) : bps.gt(MAX_BPS) ? new BN19(MAX_BPS) : bps;
}
...
var BalancedStrategyBuilder = class {
  ...
  buildRebalanceStrategyParameters() {
    ...
    let totalXAmountOut = new BN20(this.positionData.totalXAmount);
    let totalYAmountOut = new BN20(this.positionData.totalYAmount);
    totalXAmountOut = totalXAmountOut.add(new BN20(this.positionData.feeX));
    totalYAmountOut = totalYAmountOut.add(new BN20(this.positionData.feeY));
    ...
    const minDeltaId = new BN20(binPerBid).neg();
    const maxDeltaId = new BN20(binPerAsk);
    ...
    const withdrawParam = {
      minBinId: new BN20(this.positionData.lowerBinId),
      maxBinId: new BN20(this.positionData.upperBinId),
      bps: MAX_BPS
    };
    return {
      shouldClaimFee: true,
...
var DLMM = class _DLMM {
  ...
  static async createMultiple(connection, dlmmList, opt) {
    ...
    }).filter(({ mintAddress }) => mintAddress !== PublicKey10.default);
  ...
  static async getAllLbPairPositionsByUser(connection, userPubKey, opt) {
    ...
    lbPairKeys.forEach((lbPair, idx) => {
      ...
      if (!lbPairState.rewardInfos[0].mint.equals(PublicKey10.default)) {
      ...
      if (!lbPairState.rewardInfos[1].mint.equals(PublicKey10.default)) {
  ...
  async getLbPairLockInfo(lockDurationOpt) {
    ...
    const minLockReleasePoint = currentPoint.add(new BN21(lockDuration));
  ...
  async getPositionsByUserAndLbPair(userPubKey) {
    ...
    const lbPairAndBinArrays = await chunkedGetMultipleAccountInfos(
      this.program.provider.connection,
      [this.pubkey, SYSVAR_CLOCK_PUBKEY2, ...binArrayPubkeyArrayV2]
    );
  ...
  async initializePositionAndAddLiquidityByStrategy({
    ...
    const minBinArrayIndex = binIdToBinArrayIndex(new BN21(minBinId));
    const maxBinArrayIndex = binIdToBinArrayIndex(new BN21(maxBinId));
  ...
  async initializePositionAndAddLiquidityByWeight({
    ...
    preInstructions.push(initializePositionIx);
    const lowerBinArrayIndex = binIdToBinArrayIndex(new BN21(lowerBinId));
    ...
    const upperBinArrayIndex = BN21.max(
      lowerBinArrayIndex.add(new BN21(1)),
      binIdToBinArrayIndex(new BN21(upperBinId))
    );
  ...
  async addLiquidityByWeight({
    ...
    const lowerBinArrayIndex = binIdToBinArrayIndex(
      new BN21(positionAccount.lowerBinId)
    );
    const [binArrayLower] = deriveBinArray(
      this.pubkey,
      lowerBinArrayIndex,
    ...
    const upperBinArrayIndex = BN21.max(
      lowerBinArrayIndex.add(new BN21(1)),
      binIdToBinArrayIndex(new BN21(positionAccount.upperBinId))
    );
    const [binArrayUpper] = deriveBinArray(
      this.pubkey,
      upperBinArrayIndex,
  ...
  async removeLiquidity({
    ...
    const feeOwnerTokenX = getAssociatedTokenAddressSync2(
      this.lbPair.tokenXMint,
      walletToReceiveFee,
      true,
      this.tokenX.owner
    );
    const feeOwnerTokenY = getAssociatedTokenAddressSync2(
      this.lbPair.tokenYMint,
      walletToReceiveFee,
      true,
      this.tokenY.owner
    ...
    const createFeeOwnerTokenXIx = createAssociatedTokenAccountIdempotentInstruction2(
      user,
      feeOwnerTokenX,
      walletToReceiveFee,
      this.lbPair.tokenXMint,
    ...
    const createFeeOwnerTokenYIx = createAssociatedTokenAccountIdempotentInstruction2(
      user,
      feeOwnerTokenY,
      walletToReceiveFee,
      this.lbPair.tokenYMint,
      ...
      if (shouldClaimAndClose) {
        ...
        for (let i = 0; i < 2; i++) {
          ...
          if (!rewardInfo || rewardInfo.mint.equals(PublicKey10.default))
            continue;
          const userRewardAccount = getAssociatedTokenAddressSync2(
            rewardInfo.mint,
            user,
            true,
            this.rewards[i].owner
          );
          const createUserRewardAccountIx = createAssociatedTokenAccountIdempotentInstruction2(
            user,
            userRewardAccount,
            user,
            rewardInfo.mint,
          ...
          const claimRewardIx = await this.program.methods.claimReward2(new BN21(i), lowerBinId, upperBinId, {
            slices: slices2
          }).accountsPartial({
            lbPair: this.pubkey,
            sender: user,
  ...
  swapQuoteExactOut(outAmount, swapForY, allowedSlippage, binArrays, maxExtraBinArrays = 0) {
    ...
    let actualInAmount = new BN21(0);
    let feeAmount = new BN21(0);
    let protocolFeeAmount = new BN21(0);
    while (!outAmountLeft.isZero()) {
      ...
      if (!outAmountLeft.isZero()) {
        if (swapForY) {
          activeId = activeId.sub(new BN21(1));
        } else {
          activeId = activeId.add(new BN21(1));
        }
      }
    }
    ...
    const maxInAmount = actualInAmount.mul(new BN21(BASIS_POINT_MAX).add(allowedSlippage)).div(new BN21(BASIS_POINT_MAX));
    if (maxExtraBinArrays > 0 && maxExtraBinArrays <= MAX_EXTRA_BIN_ARRAYS) {
      ...
      while (extraBinArrays.length < maxExtraBinArrays) {
        ...
        if (binArrayAccountToSwapExisted) {
          ...
          if (swapForY) {
            activeId = lowerBinId.sub(new BN21(1));
          } else {
            activeId = upperBinId.add(new BN21(1));
          }
        }
      }
  ...
  swapQuote(inAmount, swapForY, allowedSlippage, binArrays, isPartialFill, maxExtraBinArrays = 0) {
    ...
    const minOutAmount = transferFeeExcludedAmountOut.mul(new BN21(BASIS_POINT_MAX).sub(allowedSlippage)).div(new BN21(BASIS_POINT_MAX));
    return {
  ...
  async seedLiquidity(owner, seedAmount, curvature, minPrice, maxPrice, base, payer, feeOwner, operator, lockReleasePoint, shouldSeedPositionOwner = false) {
    let tokenOwnerProveAssociatedTokenAccountLamports = new BN21(0);
    let totalPositionCount = new BN21(0);
    let totalPositionLamports = new BN21(0);
    ...
    const decompressMultiplier = findOptimumDecompressMultiplier(
      binDepositAmount,
      new BN21(this.tokenX.mint.decimals)
    );
    ...
    const positionCount = getPositionCount(minBinId, maxBinId.sub(new BN21(1)));
    ...
    for (let i = 0; i < positionCount.toNumber(); i++) {
      const lowerBinId = minBinId.add(DEFAULT_BIN_PER_POSITION.mul(new BN21(i)));
      const upperBinId = lowerBinId.add(DEFAULT_BIN_PER_POSITION).sub(new BN21(1));
      const binArrayAccountMetas = getBinArrayAccountMetasCoverage(
        lowerBinId,
        upperBinId,
        this.pubkey,
      ...
      for (let i2 = 0; i2 < binArrayAccountMetas.length; i2++) {
        ...
        if (!account && !appendedInitBinArrayIx.has(pubkey)) {
          totalBinArraysCount = totalBinArraysCount.add(new BN21(1));
      ...
      if (!positionAccount) {
        totalPositionCount = totalPositionCount.add(new BN21(1));
      ...
      ).liquidityShares.reduce((total, cur) => total.add(cur), new BN21(0)).gt(new BN21(0));
  ...
  async seedLiquiditySingleBin(payer, base, seedAmount, price, roundingUp, positionOwner, feeOwner, operator, lockReleasePoint, shouldSeedPositionOwner = false) {
    ...
    const binId = new BN21(binIdNumber);
  ...
  async syncWithMarketPrice(marketPrice, owner) {
    ...
    const fromBinArrayIndex = binIdToBinArrayIndex(new BN21(activeBinId));
    const swapForY = marketPriceBinId < activeBinId;
    const toBinArrayIndex = findNextBinArrayIndexWithLiquidity(
      swapForY,
      new BN21(activeBinId),
    ...
    const binArrayBitMapExtensionPubkey = isOverflowDefaultBinArrayBitmap(
      new BN21(marketPriceBinArrayIndex)
  ...
  isSwapDisabled(swapInitiator) {
    ...
    if (this.lbPair.pairType == 1 /* Permissioned */) {
      ...
      const activationPoint = !this.lbPair.preActivationSwapAddress.equals(PublicKey10.default) && this.lbPair.preActivationSwapAddress.equals(swapInitiator) ? preActivationSwapPoint : this.lbPair.activationPoint;
  ...
  async decreasePositionLength(position, side, length, allowParallelExecution = true) {
    ...
    for (let i = length.toNumber(); i > 0; i -= MAX_RESIZE_LENGTH.toNumber()) {
      ...
      const setCuIx = ComputeBudgetProgram3.setComputeUnitLimit({
  ...
  async simulateRebalancePositionWithStrategy(rebalancePosition, rebalanceStrategy) {
    ...
    const simulationResult = await rebalancePosition.simulateRebalance(
      ...
      new BN21(this.lbPair.binStep),
      new BN21(this.tokenX.mint.decimals),
      new BN21(this.tokenY.mint.decimals),
      withdraws,
      deposits
    );
  ...
  async createInitAndExtendPositionIx(lowerBinId, upperBinId, basePositionWidth, user, position) {
    ...
    if (extendedBinCount.gt(new BN21(0))) {
  ...
  static async processPosition(program, lbPair, clock, position, baseMint, quoteMint, rewardMint0, rewardMint1, binArrayMap) {
    ...
    bins.forEach((bin, idx) => {
      ...
      const newFeeX = posShare.isZero() ? new BN21(0) : mulShr(
        posShares[idx].shrn(SCALE_OFFSET),
        bin.feeAmountXPerTokenStored.sub(feeInfo.feeXPerTokenComplete),
        SCALE_OFFSET,
        1 /* Down */
      );
      const newFeeY = posShare.isZero() ? new BN21(0) : mulShr(
        posShares[idx].shrn(SCALE_OFFSET),
      ...
      for (let j = 0; j < claimableRewardsInBin.length; j++) {
        ...
        if (!pairRewardInfo.mint.equals(PublicKey10.default)) {
          ...
          if (bin.binId == lbPair.activeId && !bin.supply.isZero()) {
            ...
            const rewardPerTokenStoredDelta = pairRewardInfo.rewardRate.mul(delta2).div(new BN21(15)).div(liquiditySupply);
          ...
          const newReward = posShares[idx].isZero() ? new BN21(0) : mulShr(
            delta,
            posShares[idx].shrn(SCALE_OFFSET),
            SCALE_OFFSET,
            1 /* Down */
    ...
    let rewardOneExcludeTransferFee = new BN21(0);
    let rewardTwoExcludeTransferFee = new BN21(0);
    if (rewardMint0) {
    ...
    const totalXAmountExcludeTransferFee = calculateTransferFeeExcludedAmount(
      new BN21(totalXAmount.floor().toString()),
      baseMint,
      currentEpoch
    ).amount;
    const totalYAmountExcludeTransferFee = calculateTransferFeeExcludedAmount(
      new BN21(totalYAmount.floor().toString()),
      quoteMint,
      currentEpoch
    ).amount;
    return {
  ...
  static getBinsBetweenLowerAndUpperBound(lbPairKey, lbPair, lowerBinId, upperBinId, baseTokenDecimal, quoteTokenDecimal, binArrayMap, programId) {
    const lowerBinArrayIndex = binIdToBinArrayIndex(new BN21(lowerBinId));
    const upperBinArrayIndex = binIdToBinArrayIndex(new BN21(upperBinId));
    let bins = [];
    const ZERO = new BN21(0);
    for (let binArrayIndex = lowerBinArrayIndex.toNumber(); binArrayIndex <= upperBinArrayIndex.toNumber(); binArrayIndex++) {
      const binArrayIndexBN = new BN21(binArrayIndex);
      const binArrayKey = deriveBinArray(
        lbPairKey,
        binArrayIndexBN,
        programId
  ...
  async getBins(lbPairPubKey, lowerBinId, upperBinId, baseTokenDecimal, quoteTokenDecimal, lowerBinArray, upperBinArray) {
    ...
    const binArrayPubkeys = range(
      ...
      (i) => deriveBinArray(lbPairPubKey, new BN21(i), this.program.programId)[0]
    );
  ...
  async createClaimBuildMethod({
    ...
    for (const {
      ...
      const binArrayAccountsMeta = getBinArrayAccountMetasCoverage(
        new BN21(chunkedLowerBinId),
        new BN21(chunkedUpperBinId),
        this.pubkey,
        this.program.programId
      );
      for (let i = 0; i < 2; i++) {
        ...
        const createUserTokenIx = createAssociatedTokenAccountIdempotentInstruction2(
          owner,
          userRewardToken,
          owner,
          rewardInfo.mint,
        ...
        const claimTransaction = await this.program.methods.claimReward2(new BN21(i), chunkedLowerBinId, chunkedUpperBinId, {
          slices
        }).accountsPartial({
          lbPair: this.pubkey,
          sender: owner,
  ...
  async createClaimSwapFeeMethod({
    ...
    for (const {
      ...
      const walletToReceiveFee = feeOwner.equals(PublicKey10.default) ? owner : feeOwner;
      const preInstructions = [];
      const userTokenX = getAssociatedTokenAddressSync2(
        this.lbPair.tokenXMint,
        walletToReceiveFee,
